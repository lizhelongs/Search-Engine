...
public class QryParser {
  ...
  private static Qry createOperator (String operatorName) {
    ...
    String[] opArr = operatorNameLowerCase.split("/");
    operatorNameLowerCase = opArr[0];
    int dist = 0;
    if(opArr.length > 1) {
      dist = Integer.valueOf(opArr[1]);
    }
    ...
    for (int i = q.args.size() - 1; i >= 0; i--) {
      ...
      if (q_i_after == null) {
        ...
        if (q_i_before != q_i_after) {
          q_i_after.weight = q_i_before.weight;
...
//    }
    ...
    while (queryString.length() > 0) {
      ...
      if(Character.isDigit(queryString.charAt(0))){  // weighted
        p = popTerm(queryString);  // pop the weight
        weight = p.getPopped();
      ...
      for (int i=0; i<qargs.length; i++) {
        ...
        if(!weight.equals("")) {
          queryTree.args.get(queryTree.args.size()-1).weight = Double.valueOf(weight);
          queryTree.totalWeight += Double.valueOf(weight);
        }
      }
    }
