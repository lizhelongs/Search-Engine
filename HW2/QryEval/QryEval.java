...
public class QryEval {
  ...
  static int trecEvalOutputLength = 0;
  ...
  public static void main(String[] args) throws Exception {
    ...
    trecEvalOutputLength = Integer.valueOf(parameters.get("trecEvalOutputLength"));
    //  Perform experiments.
...
//    System.out.println(queryName + ":  ");
    int qid = Integer.valueOf(queryName.strip());
//    File outFile = new File(trecEvalOutputPath);
//    PrintWriter pw = new PrintWriter(new FileOutputStream(outFile, true));
    PrintWriter pw = new PrintWriter(new FileOutputStream(trecEvalOutputPath, true));
    if (result.size() < 1) {
      pw.printf("%d Q0 dummy 1 0 run-1\n", qid);
    } else {
      result.sort();
      int rank = 1;
      int len = Math.min(trecEvalOutputLength, result.size());
      for (int i = 0; i < len; i++) {
        pw.printf("%d Q0 %s %s %.10f run-1\n", qid, Idx.getExternalDocid(result.getDocid(i)), rank++, result.getDocidScore(i));
      }
    }
    pw.close();
  }
